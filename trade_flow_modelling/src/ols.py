        ...
        **kwargs
        ...
        if method == "pinv":
            if not (hasattr(self, 'pinv_wexog') and
                    ...
                    hasattr(self, 'rank')):
                s_important_1 = time.time()
                self.pinv_wexog, singular_values = pinv_extended(self.wexog)
                e_important_1 = time.time()
                important.append(e_important_1 - s_important_1)

                s_useless_1 = time.time()
                self.normalized_cov_params = np.dot(
                    self.pinv_wexog, np.transpose(self.pinv_wexog))
                e_useless_1 = time.time()
                useless.append(e_useless_1 - s_useless_1)

                # Cache these singular values for use later.
                s_useless_2 = time.time()
                self.wexog_singular_values = singular_values
                self.rank = np.linalg.matrix_rank(np.diag(singular_values))
                e_useless_2 = time.time()
                useless.append(e_useless_2 - s_useless_2)

            s_important_2 = time.time()
            beta = np.dot(self.pinv_wexog, self.wendog)
            print(f"BETA: {beta} | {self.pinv_wexog.shape}| {self.wendog.shape}| {beta.shape}")
            e_important_2 = time.time()
            important.append(e_important_2 - s_important_2)
            ...
            s_useless_3 = time.time()
            self.pinv_wexog = np.linalg.pinv(self.wexog)
            e_useless_3 = time.time()
            useless.append(e_useless_3 - s_useless_3)
            # used in ANOVA
            s_important_2 = time.time()
        ...
        if isinstance(self, OLS):
            ...
            lfit = RegressionResults(
                ...
                **kwargs)
        s_wrapper = time.time()
        wrapper = RegressionResultsWrapper(lfit)
        e_wrapper = time.time()
        t_wrapper = e_wrapper - s_wrapper
        e_total = time.time()
        t_total = e_total - s_total
        print(f"Important: {sum(important)} s ({important})")
        print(f"Useless: {sum(useless)} s ({useless})")
        print(f"Result: {t_result} s | wrapper: {t_wrapper} s")
        print(f"Total fit(): {t_total}")
        return wrapper, sum(useless)

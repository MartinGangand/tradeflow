...
from scripts.tests.test_utils import (
    mock_chrome_with_html_page,
    mock_response_with_source,
    mock_response_with_wheel,
    prepare_directory_with_files,
    read_html_page_from_datasets,
)
from scripts.utils import file_names_with_prefixes
from scripts.verify_uploaded_package_content import (
    compare_expected_vs_actual_files,
    display_name,
    expected_wheel_shared_libraries_extension,
    main,
    verify_source,
    verify_source_url,
    verify_wheel,
...
class TestVerifySource:
    @pytest.mark.parametrize(
        "file_names,expected_python_files,expected_cpp_files",
        [
            (
            ...
            (
                ["package/time_series.py", "simulation.cpp"],
    ...
    def test_verify_source_should_not_raise_exception(self, mocker, file_names, expected_python_files, expected_cpp_files):
        ...
        assert (
            verify_source(
                ...
                expected_cpp_files=expected_cpp_files,
            )
            is None
        )
    ...
    def test_verify_source_should_raise_exception_when_invalid_url(self, source_name):
        ...
        with pytest.raises(Exception) as ex:
            verify_source(
                ...
                package_name="package",
                version="0.0.1",
                expected_python_files=["ar_model.py"],
                expected_cpp_files=["simulation.cpp"],
            )
    ...
    @pytest.mark.parametrize(
        "file_names,expected_python_files,expected_cpp_files,expected_matched_python_files",
        [
            ...
            (
                ["time_series.py", "ar_model.py", "model.py"],
                ["time_series.py", "ar_model.py"],
                [],
                ["time_series.py", "ar_model.py", "model.py"],
            ),
    ...
    def test_verify_source_should_raise_exception_when_incorrect_python_files(
        self,
        mocker,
        file_names,
        expected_python_files,
        expected_cpp_files,
        expected_matched_python_files,
    ):
        file_names_with_setup = [os.path.join("package-0.0.1", file_name) for file_name in file_names] + [os.path.join("package-0.0.1", "setup.py")]
        mock_request_get = mock_response_with_source(mocker=mocker, file_names=file_names_with_setup)
    ...
    @pytest.mark.parametrize(
        "file_names,expected_python_files,expected_cpp_files,expected_matched_cpp_files",
        [
            ...
            (
                ["time_series.py", "simulation.cpp"],
                ["time_series.py"],
                ["simulation.cpp", "simulation.h"],
                ["simulation.cpp"],
            ),
            (
                ["time_series.py", "simulation.cpp", "simulation.h", "simulation2.cpp"],
                ["time_series.py"],
                ["simulation.cpp", "simulation.h"],
                ["simulation.cpp", "simulation.h", "simulation2.cpp"],
            ),
            (
                [
                    "package/time_series.py",
    ...
    def test_verify_source_should_raise_exception_when_incorrect_cpp_files(
        self,
        mocker,
        file_names,
        expected_python_files,
        expected_cpp_files,
        expected_matched_cpp_files,
    ):
        file_names_with_setup = [os.path.join("package-0.0.1", file_name) for file_name in file_names] + [os.path.join("package-0.0.1", "setup.py")]
        mock_request_get = mock_response_with_source(mocker=mocker, file_names=file_names_with_setup)
        mocker.patch("requests.get", return_value=mock_request_get)
...
class TestVerifySourceUrl:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "package2-0.0.1.tar.gzpackage-0.0.1",
            "0.0.1.tar.gz",
            "package-0.0.1",
        ],
    )
...
class TestVerifyWheel:
    @pytest.mark.parametrize(
        "wheel_name,file_names,expected_shared_libraries,expected_python_files",
        [
            (
                WHEEL_CP312_MANYLINUX,
                ["package/ar_model.py", "package/lib1.so"],
                ["package/lib1"],
                ["package/ar_model.py"],
            ),
            (
                WHEEL_CP312_MUSLLINUX,
                ["time_series.py", "ar_model.py", "lib1.so", "lib2.so"],
                ["lib1", "lib2"],
    ...
    def test_verify_wheel_should_not_raise_exception(
        self,
        mocker,
        wheel_name,
        file_names,
        ...
        assert (
            verify_wheel(
                ...
                version=VERSION,
                expected_shared_libraries=expected_shared_libraries,
                expected_python_files=expected_python_files,
            )
            is None
        )
    ...
    def test_verify_wheel_should_raise_exception_when_invalid_url(self):
        ...
        with pytest.raises(Exception) as ex:
            verify_wheel(
                ...
                package_name=PACKAGE_NAME,
                version=VERSION,
                expected_shared_libraries=["lib1"],
                expected_python_files=["ar_model.py"],
            )

        expected_error_message = r"expected wheel url 'https://test-files.pythonhosted.org/packages/package-0.0-cp312-cp312-manylinux_2_17_x86_64.manylinux2014_x86_64.whl' to match the pattern 'package-0.0.1-[^'\"\s]+\.whl\b'"
        assert str(ex.value) == expected_error_message

    @pytest.mark.parametrize(
        "wheel_name,file_names,expected_shared_libraries,expected_python_files,expected_matched_shared_libraries,expected_shared_library_extension",
        [
            (
                WHEEL_CP312_MUSLLINUX,
                ["ar_model.py", "lib1.so", "lib2.so"],
                ["lib1"],
                ["ar_model.py"],
                ["lib1.so", "lib2.so"],
                "so",
    ...
    def test_verify_wheel_should_raise_exception_when_incorrect_shared_libraries(
        self,
        mocker,
        wheel_name,
        file_names,
        expected_shared_libraries,
        expected_python_files,
        expected_matched_shared_libraries,
    ...
    @pytest.mark.parametrize(
        "wheel_name,file_names,expected_shared_libraries,expected_python_files,expected_matched_python_files",
        [
            (
                WHEEL_CP312_MANYLINUX,
                ["lib1.so"],
            ...
            (
                WHEEL_CP312_WIN,
                ["time_series.py", "ar_model.py", "lib1.dll"],
                ["lib1"],
                ["time_series.py"],
                ["time_series.py", "ar_model.py"],
            ),
    ...
    def test_verify_wheel_should_raise_exception_when_incorrect_python_files(
        self,
        mocker,
        wheel_name,
        file_names,
...
class TestVerifyWheelUrl:
    @pytest.mark.parametrize(
        ...
        [
            ...
            "package-0.0.1-cp312-cp312-macosx_11_0_arm64.whlpackage-0.0.1-cp312-cp312-win_amd64.whl",
    ...
    @pytest.mark.parametrize(
        "wheel_name",
        [
            "package-0.0.2-any.whlpackage-0.0.1-any.wh",
...
class TestExpectedWheelSharedLibrariesExtension:
    @pytest.mark.parametrize(
        "wheel_url,expected_shared_libraries_extension",
        [
    ...
    def test_expected_wheel_shared_libraries_extension_should_raise_exception(self, wheel_url):
        ...
        assert str(ex.value) == "The wheel name does not contain 'linux', 'macosx' nor 'win'"
...
class TestCompareExpectedVsActualFiles:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ["time_series.py", "ar_model.py", "LICENSE"],
                [
                    "LICENSE",
                    "time_series.py",
                    "ar_model.py",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ["time_series.py", "ar_model.py"],
                ["time_series.py", "ar_model.py", "setup.py"],
            ),
        ],
    )
...
class TestDisplayName:
    @pytest.mark.parametrize(
        "url,expected_display_name",
        [
            (
                "https://test-files/package-0.0.1-cp312-cp312-win_amd64.whl",
            ...
            (
                "https://test-files/invalid_name.whl",
                "https://test-files/invalid_name.whl",
            ),
        ],
    )
...
class TestMain:
    ...
    def main_setup_and_tear_down(self):
        ...
        prepare_directory_with_files(
            directory_path=package_directory,
            file_names=["time_series.py", "ar_model.py"],
        )

        package_cpp_directory = self.ROOT_REPOSITORY.joinpath(self.LIBRARIES_DIRECTORY_NAME, "cpp", PACKAGE_NAME)
        prepare_directory_with_files(
            directory_path=package_cpp_directory,
            file_names=["simulation.cpp", "simulation.h"],
        )
    ...
    def mock_request_get_valid(mocker: MockerFixture, url: str) -> MagicMock:
        if url == f"{SOURCE_URL_START}/packages/{PACKAGE_NAME}-{VERSION}.tar.gz":
            ...
            cpp_file_names = file_names_with_prefixes(
                ["simulation.cpp", "simulation.h"],
                PACKAGE_AND_VERSION,
                "lib",
                "cpp",
            ...
            return mock_response_with_source(
                mocker=mocker,
                file_names=root_file_names + package_file_names + cpp_file_names,
            )
        elif url == f"{WHEEL_URL_START}/{WHEEL_CP312_WIN}":
            return mock_response_with_wheel(
                mocker=mocker,
                file_names=file_names_with_prefixes(["time_series.py", "ar_model.py", "lib1.dll"], PACKAGE_NAME),
            )
        elif url == f"{WHEEL_URL_START}/{WHEEL_CP312_MUSLLINUX}":
            return mock_response_with_wheel(
                mocker=mocker,
                file_names=file_names_with_prefixes(["time_series.py", "ar_model.py", "lib1.so"], PACKAGE_NAME),
            )
            ...
            return mock_response_with_wheel(
                mocker=mocker,
                file_names=file_names_with_prefixes(["time_series.py", "ar_model.py", "lib1.dylib"], PACKAGE_NAME),
            )
    ...
    def mock_request_get_invalid(mocker: MockerFixture, url: str) -> MagicMock:
        if url == f"{SOURCE_URL_START}/packages/source-invalid-missing-cpp-files/{PACKAGE_NAME}-{VERSION}.tar.gz":
            ...
            return mock_response_with_wheel(
                mocker=mocker,
                file_names=file_names_with_prefixes(
                    ["time_series.py", "ar_model.py", "lib1.so", "lib2.so", "lib3.soo"],
                    PACKAGE_NAME,
                ),
            )
            ...
            return mock_response_with_wheel(
                mocker=mocker,
                file_names=file_names_with_prefixes(["time_series.py", "ar_model.py"], PACKAGE_NAME),
            )
        elif url == f"{WHEEL_URL_START}/{PACKAGE_NAME}-{VERSION}-win-missing-python-files.whl":
            return mock_response_with_wheel(
                mocker=mocker,
                file_names=file_names_with_prefixes(["time_series.py", "lib1.dll"], PACKAGE_NAME),
            )
        elif url == f"{WHEEL_URL_START}/{PACKAGE_NAME}-{VERSION}-musllinux-too-many-python-files.whl":
            return mock_response_with_wheel(
                mocker=mocker,
                file_names=file_names_with_prefixes(
                    ["time_series.py", "ar_model.py", "ar_model2.py", "lib1.so"],
                    PACKAGE_NAME,
                ),
            )

    def test_main_valid(self, mocker, capsys, file_regression):
        mocked_chrome = mock_chrome_with_html_page(
            mocker=mocker,
            html_page_content=read_html_page_from_datasets(file_name="html_page_test_main_valid.html"),
        )
        mocker.patch("selenium.webdriver.Chrome", return_value=mocked_chrome)
        mocker.patch(
            "requests.get",
            side_effect=lambda url: self.mock_request_get_valid(mocker=mocker, url=url),
        )
        ...
        nb_errors = main(
            ...
            package_name=PACKAGE_NAME,
            version=VERSION,
            expected_nb_wheels=4,
            expected_shared_libraries=self.EXPECTED_SHARED_LIBRARIES,
            root_repository=self.ROOT_REPOSITORY,
            main_package_directory=self.MAIN_PACKAGE_DIRECTORY,
            subpackage_directories=self.SUBPACKAGES_DIRECTORIES,
        )
    ...
    def test_main_should_raise_exception_when_2_sources_instead_of_1(self, mocker, capsys, file_regression):
        mocked_chrome = mock_chrome_with_html_page(
            mocker=mocker,
            html_page_content=read_html_page_from_datasets(file_name="html_page_test_main_2_sources.html"),
        )
        mocker.patch("selenium.webdriver.Chrome", return_value=mocked_chrome)
    ...
    def test_main_should_raise_exception_when_3_wheels_instead_of_4(self, mocker, capsys, file_regression):
        mocked_chrome = mock_chrome_with_html_page(
            mocker=mocker,
            html_page_content=read_html_page_from_datasets(file_name="html_page_test_main_3_wheels.html"),
        )
        mocker.patch("selenium.webdriver.Chrome", return_value=mocked_chrome)
    ...
    def test_main_invalid(self, mocker, capsys, file_regression):
        mocked_chrome = mock_chrome_with_html_page(
            mocker=mocker,
            html_page_content=read_html_page_from_datasets(file_name="html_page_test_main_invalid.html"),
        )
        mocker.patch("selenium.webdriver.Chrome", return_value=mocked_chrome)
        mocker.patch(
            "requests.get",
            side_effect=lambda url: self.mock_request_get_invalid(mocker=mocker, url=url),
        )

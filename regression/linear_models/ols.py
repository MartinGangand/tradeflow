...
from ...constants.constants import InformationCriterion, OLSMethod
from ...utils.general_utils import check_enum_value_is_valid, get_enum_values
...
class OLS(RegressionModel):
    ...
    def fit(self, method: Literal["pinv"]):
        method = check_enum_value_is_valid(enum=OLSMethod, value=method, parameter_name="method",
                                           is_none_valid=False)
        match method:
            case OLSMethod.PINV:
            ...
            case _:
                raise IllegalValueException(
                    f"The method '{method}' for the OLS fit is not valid, it must be among {get_enum_values(enum=OLSMethod)}.")
        # TODO: QR method
...
class OLSResults:
    ...
    def info_criterion(self, criterion: Literal["aic", "bic", "hqic"]) -> float:
        criterion = check_enum_value_is_valid(enum=InformationCriterion, value=criterion,
                                              parameter_name="criterion", is_none_valid=False)
        match criterion:
            ...
            case _:
                raise IllegalValueException(
                    f"The information criterion '{criterion}' is not valid, it must be among {get_enum_values(enum=InformationCriterion)}.")

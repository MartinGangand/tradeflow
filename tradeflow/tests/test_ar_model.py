...
from tradeflow.datasets import trade_signs_btcusdt_20240720, trade_signs_sample
...
from tradeflow.tests.test_time_series import (
    ...
    def test_init_should_raise_exception_when_invalid_order_selection_method(self, signs_sample):
        with pytest.raises(EnumValueException) as ex:
            AR(
                signs=signs_sample,
                max_order=6,
                order_selection_method="invalid_order_selection_method",
            )
...
class TestFit:
    ...
    def test_fit(self, ar_model_with_max_order_6, method, num_regression):
        ...
        self._check_fitted_parameters(
            ar_model=ar_model_with_max_order_6,
            num_regression=num_regression,
            absolute_tolerance=1e-10,
        )

    @pytest.mark.parametrize(
        "method,start_parameters",
        [
            (
                FitMethodAR.MLE_WITHOUT_CST,
                [
                    0.20793670441358317,
                    0.15625334330632215,
                    0.08328570101676176,
    ...
    def test_fit_with_mle_method(
        self,
        mocker,
        ar_model_with_max_order_6,
        method,
        start_parameters,
        num_regression,
    ):
        mocker.patch.object(
            ar_model_with_max_order_6,
            "_compute_start_parameters",
            return_value=start_parameters,
        )
    ...
    def test_fit_should_raise_exception_when_time_series_is_non_stationary_and_check_stationarity_is_true(self, ar_model_non_stationary_with_max_order_1):
        with pytest.raises(NonStationaryTimeSeriesException) as ex:
            ar_model_non_stationary_with_max_order_1.fit(
                method=FitMethodAR.YULE_WALKER.value,
                significance_level=SIGNIFICANCE_LEVEL,
                check_stationarity=True,
                check_residuals_not_autocorrelated=False,
            )
    ...
    def test_fit_should_raise_exception_when_residuals_are_autocorrelated_and_check_residuals_not_autocorrelated_is_true(self, mocker, ar_model_with_max_order_6):
    ...
    def test_fit_should_not_raise_exception_when_residuals_are_not_autocorrelated_and_check_residuals_not_autocorrelated_is_true(self, mocker, ar_model_with_max_order_6):
    ...
    def test_fit_should_not_raise_exception_when_residuals_are_autocorrelated_and_check_residuals_not_autocorrelated_is_false(self, mocker, ar_model_with_max_order_6):
        autocorrelated_resid = generate_autoregressive(size=10_000, parameters=[0.04, 0.01], sigma=1, seed=1)
...
class TestGetModelXY:
    def test_get_model_x_y_without_cst_parameter(self):
        ...
        expected_y = np.asarray([[4.0], [5.0], [6.0], [7.0], [8.0]])
...
class TestLogLikelihood:
    PARAMETERS_WITHOUT_CST = np.asarray(
        [
            0.20793670441358317,
            0.15625334330632215,
            0.08328570101676176,
    ...
    @pytest.mark.parametrize(
        "has_cst_parameter,parameters,expected_log_likelihood",
        [
    ...
    def test_log_likelihood(
        ...
        self._set_model_order_start_idx_x_y(
            ar_model=ar_model_with_max_order_6,
            order=6,
            has_cst_parameter=has_cst_parameter,
        )
    ...
    @pytest.mark.parametrize(
        "has_cst_parameter,parameters",
        [(False, PARAMETERS_WITHOUT_CST), (True, PARAMETERS_WITH_CST)],
    )
...
class TestSelectOrder:
    @pytest.mark.parametrize(
        "max_order,order_selection_method,expected_order",
        [(499, "pacf", 6), (1, "pacf", 1)],
    )
...
class TestSetParameters:
    @pytest.mark.parametrize(
        "parameters,has_cst_parameter,expected_parameters,expected_constant_parameter",
        [
            (np.asarray([0.21, 0.12, 0.04]), False, np.asarray([0.21, 0.12, 0.04]), 0),
            (
                np.asarray([0.035, 0.21, 0.12, 0.04]),
                True,
                np.asarray([0.21, 0.12, 0.04]),
                0.035,
            ),
        ],
    )
    def test_set_parameters(
        self,
        ar_model_with_order_3,
        parameters,
        has_cst_parameter,
        expected_parameters,
        expected_constant_parameter,
    ):
    ...
    @pytest.mark.parametrize(
        "parameters,has_cst_parameter,expected_exception_message",
        [
            ...
            (
                np.asarray([0.035, 0.21, 0.12, 0.04]),
                False,
                "Expected 3 parameters, but got 4.",
            ),
            (
                np.asarray([0.035, 0.21, 0.12, 0.04, 0.01]),
                True,
                "Expected 4 parameters (including constant term), but got 5.",
            ),
            (
                np.asarray([0.21, 0.12, 0.04]),
                True,
                "Expected 4 parameters (including constant term), but got 3.",
            ),
        ],
    )
    def test_set_parameters_should_raise_exception(
        self,
        ar_model_with_order_3,
        parameters,
        has_cst_parameter,
...
class TestSimulate:
    ...
    def fitted_model(self, ar_model_with_max_order_6):
        return ar_model_with_max_order_6.fit(
            method=FitMethodAR.YULE_WALKER.value,
            significance_level=SIGNIFICANCE_LEVEL,
            check_stationarity=True,
            check_residuals_not_autocorrelated=True,
    ...
    def test_simulate(self, fitted_model, num_regression):
        ...
        # Results are from statsmodels.
        num_regression.check(
            {"simulated_signs": actual_simulation},
            default_tolerance=dict(atol=0, rtol=0),
        )
...
class TestSimulationSummary:
    ...
    def test_simulation_summary(self, signs_sample, plot_acf, plot_pacf):
        ...
        res = ar_model.simulation_summary(
            plot_acf=plot_acf,
            plot_pacf=plot_pacf,
            log_scale=False,
        ...
        expected_simulation_summary_df = pd.concat([expected_training_stats_df, expected_simulation_stats_df], axis=1).round(decimals=2)

        assert_frame_equal(
            left=actual_simulation_summary_df,
            right=expected_simulation_summary_df,
            check_dtype=True,
    ...
    @pytest.mark.parametrize(
        "fit_method",
        [
            FitMethodAR.YULE_WALKER,
            FitMethodAR.BURG,
            FitMethodAR.CMLE_WITHOUT_CST,
            FitMethodAR.CMLE_WITH_CST,
            FitMethodAR.MLE_WITHOUT_CST,
            FitMethodAR.MLE_WITH_CST,
        ],
    )
    def test_simulation_summary_regression(self, signs_btcusdt, fit_method):
        ...
        summary_df = ar_model.simulation_summary(plot_acf=False, plot_pacf=False, percentiles=(50.0, 75.0, 95.0, 99.0))
        ...
        assert_allclose(
            actual=summary_df.loc["pct_buy (%)"]["Simulation"],
            desired=expected_pct_buy,
            rtol=0,
            atol=1.0,
            equal_nan=False,
        )
        assert_allclose(
            actual=summary_df.loc["mean_nb_consecutive_values"]["Simulation"],
            desired=summary_df.loc["mean_nb_consecutive_values"]["Training"],
            rtol=0,
            atol=0.25,
            equal_nan=False,
        )
        assert_allclose(
            actual=summary_df.loc["std_nb_consecutive_values"]["Simulation"],
            desired=summary_df.loc["std_nb_consecutive_values"]["Training"],
            rtol=0,
            atol=15,
            equal_nan=False,
        )
        ...
        assert_allclose(
            actual=summary_df.loc["Q50.0_nb_consecutive_values"]["Simulation"],
            desired=summary_df.loc["Q50.0_nb_consecutive_values"]["Training"],
            rtol=0,
            atol=1,
            equal_nan=False,
        )
        assert_allclose(
            actual=summary_df.loc["Q75.0_nb_consecutive_values"]["Simulation"],
            desired=summary_df.loc["Q75.0_nb_consecutive_values"]["Training"],
            rtol=0,
            atol=2,
            equal_nan=False,
        )
        assert_allclose(
            actual=summary_df.loc["Q95.0_nb_consecutive_values"]["Simulation"],
            desired=summary_df.loc["Q95.0_nb_consecutive_values"]["Training"],
            rtol=0,
            atol=3,
            equal_nan=False,
        )
        assert_allclose(
            actual=summary_df.loc["Q99.0_nb_consecutive_values"]["Simulation"],
            desired=summary_df.loc["Q99.0_nb_consecutive_values"]["Training"],
            rtol=0,
            atol=6,
            equal_nan=False,
        )

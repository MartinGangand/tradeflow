...
class SharedLibrariesRegistry(metaclass=Singleton):
    ...
    def _get_shared_libraries(self) -> list[SharedLibrary]:
        ...
        libtradeflow = SharedLibrary(
            name="libtradeflow",
            directory=LIBTRADEFLOW_SHARED_LIBRARY_DIRECTORY,
            functions=[simulate],
        )
        return [libtradeflow]
    ...
    def _add_shared_library(self, shared_library: SharedLibrary) -> SharedLibrariesRegistry:
        ...
        logger.info("Added shared library '%s' to the registry", shared_library.name)
        return self

    def find_shared_library(self, name: str) -> SharedLibrary:
        check_condition(
            condition=name in self._name_to_shared_library,
            exception=SharedLibraryNotFoundException(f"Shared library '{name}' not found in the registry."),
        )
...
class SharedLibrary:
    ...
    def load(self) -> ct.CDLL:
        ...
        check_condition(
            condition=shared_library_path.exists() and shared_library_path.is_file(),
        ...
        logger.info("Loaded shared library '%s'", self._name)
        self._cdll = cdll
        return self._cdll
    ...
    def get_shared_library_extension() -> str:
        ...
        check_condition(
            condition=extension is not None,
            exception=UnsupportedOsException(f"Unsupported OS '{os_name}'. Supported OS values are Linux, Darwin, and Windows."),
        )
        return extension
...
class Function:
    def __init__(
        self,
        name: str,
        argtypes: list[ct._CData | ct.POINTER(ct._CData)],
        restype: ct._CData | ct.POINTER(ct._CData),
    ) -> None:
        self._name = name
    ...
    @property
    def argtypes(self) -> tuple[ct._CData | ct.POINTER(ct._CData), ...]:
        return self._argtypes

    @property
    def restype(self) -> ct._CData | ct.POINTER(ct._CData):

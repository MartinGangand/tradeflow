...
def check_enum_value_is_valid(enum: Enum, value: str, is_none_valid: bool, parameter_name: str) -> Enum:
    if (value is None and is_none_valid):
        return value
    
    enum_value_to_enum = get_valid_enum_values(enum=enum)

    check_condition(value in enum_value_to_enum, EnumValueException(f"The value '{value}' for the {parameter_name} is not valid, it must be among {list(enum_value_to_enum.keys())}."))

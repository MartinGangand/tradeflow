...
from TradeFlowModelling.exceptions.custom_exceptions import EnumValueException
from TradeFlowModelling.utils.general_utils import check_condition, get_enum_values, check_enum_value_is_valid, is_value_within_interval_exclusive
...
def test_check_condition():
    assert check_condition(condition=True, exception=Exception("Exception message"))
...
def test_get_enum_values(enum, expected_enum_values, request):
    actual_enum_values = get_enum_values(enum=request.getfixturevalue(enum))
...
def test_check_enum_value_is_valid(enum, enum_value, is_none_valid, expected_enum, request):
    ...
    assert check_enum_value_is_valid(enum=enum, value=enum_value, is_none_valid=is_none_valid, parameter_name="enum_value") == expected_enum
...
def test_check_enum_value_is_valid_should_raise_exception(enum, enum_value, is_none_valid, expected_valid_values, request):
    ...
    with pytest.raises(EnumValueException) as ex:
        check_enum_value_is_valid(enum=enum, value=enum_value, is_none_valid=is_none_valid, parameter_name="enum_value")
